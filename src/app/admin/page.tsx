...
import {
  ...
  UserX, UserCheck, XCircle, Eye, Package
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-400 hidden sm:block" suppressHydrationWarning>{lastRefresh.toLocaleTimeString('id-ID')}</span>
          ...
          <button onClick={refreshAll} className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-3 py-2 rounded-xl text-sm font-bold transition-colors">
            <RefreshCw className="w-3.5 h-3.5" /> Refresh
          </button>
          <button onClick={() => { if (confirm('Keluar dari admin panel?')) { localStorage.removeItem('admin_authed'); window.location.href = '/'; } }} className="flex items-center gap-1.5 bg-red-50 dark:bg-red-900/20 hover:bg-red-600 text-red-600 hover:text-white dark:text-red-400 px-3 py-2 rounded-xl text-sm font-bold transition-colors border border-red-200 dark:border-red-800/50">
            <XCircle className="w-3.5 h-3.5" /> Keluar
          </button>
        </div>
      </header>
      ...
      <div className="flex min-h-[calc(100vh-65px)]">
        ...
        <aside className="w-64 shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col sticky top-[65px] h-[calc(100vh-65px)] overflow-y-auto">
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 space-y-1">
            <button onClick={refreshAll} className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              <RefreshCw className="w-4 h-4 text-slate-400" /> Refresh Data
            </button>
            <button onClick={() => { if (confirm('Keluar?')) window.location.href = '/'; }} className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">

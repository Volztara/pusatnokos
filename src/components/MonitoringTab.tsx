...
export function MonitoringTab({ showToast }: { showToast: (msg: string) => void }) {
  ...
  const [highCancelCount, setHighCancelCount] = useState(0);
  ...
  const [showFilter, setShowFilter] = useState<'all' | 'anomaly' | 'burst' | 'high_cancel' | 'normal'>('all');
  ...
  const fetchData = useCallback(async (p: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = res.status === 401 ? 'Sesi admin expired. Silakan login ulang.' : `Gagal memuat data (${res.status}).`;
        setFetchError(msg);
        return;
      }
      ...
      setHighCancelCount(data.high_cancel_count ?? 0);
  ...
  // Auto-refresh setiap 60 detik — deteksi anomali baru tanpa refresh manual
  useEffect(() => {
    const t = setInterval(() => fetchData(period), 60 * 1000);
  ...
  const doBlacklist = async () => {
    ...
    try {
      ...
      if (!r.ok || !d.success) {
        showToast(d.message ?? '❌ Gagal memblokir user. Mungkin sudah diblokir.');
        return;
      }
  ...
  const filtered = users
    .filter(u => {
      ...
      if (showFilter === 'high_cancel') return u.high_cancel && !u.is_anomaly;
      if (showFilter === 'normal') return !u.is_anomaly && !u.burst_detected && !u.high_cancel;
      return true;
    })
  ...
  // Gunakan tanggal server dari data (key pertama di dailySummary yang paling baru)
  // Fallback ke UTC agar konsisten dengan timezone server
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Error Banner ── */}
      {fetchError && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl p-4 flex items-center gap-3">
          <AlertTriangle className="w-5 h-5 text-red-500 shrink-0" />
          <div className="font-bold text-red-700 dark:text-red-400 text-sm">{fetchError}</div>
        </div>
      )}
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <div className="font-black text-red-700 dark:text-red-400 text-sm">
              🚨 {anomalyCount} user memiliki pengeluaran melebihi semua deposit!
            </div>
      ...
      {highCancelCount > 0 && (
        <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <div className="font-black text-orange-700 dark:text-orange-400 text-sm">
              🔄 {highCancelCount} user dengan cancel rate &gt;70% (min 5 order)
            </div>
            <div className="text-xs text-orange-600 dark:text-orange-400 mt-0.5">Kemungkinan testing sistem atau abuse — perlu investigasi</div>
          </div>
        </div>
      )}

      {anomalyCount === 0 && burstCount === 0 && highCancelCount === 0 && !loading && (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex gap-1.5">
              {([
                ...
                ['high_cancel', `🔄 Cancel Tinggi (${highCancelCount})`],
                ['normal', '✅ Normal'],
              ] as const).map(([val, label]) => (
                <button key={val} onClick={() => setShowFilter(val)}
                  className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition-all ${showFilter === val
              ...
              {loading ? (
                ...
                <React.Fragment key={u.user_id}>
                  <tr
                    ...
                    className={`cursor-pointer transition-colors ${u.is_blacklisted ? 'opacity-40'
                      : u.is_anomaly ? 'bg-red-50/30 dark:bg-red-900/10 hover:bg-red-50/60 dark:hover:bg-red-900/20'
                        : u.high_cancel ? 'bg-orange-50/20 dark:bg-orange-900/10 hover:bg-orange-50/40 dark:hover:bg-orange-900/20'
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-1 flex-wrap">
                        ...
                        {u.high_cancel && !u.is_anomaly && (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400">
                            <span className="w-1.5 h-1.5 rounded-full bg-orange-500 shrink-0" />Cancel Tinggi
                          </span>
                        )}
                        {!u.is_blacklisted && !u.is_anomaly && !u.burst_detected && !u.high_cancel && (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400">
                      ...
                      <td colSpan={8} className="px-6 py-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                          <div className="bg-white dark:bg-[#0d1020] rounded-2xl p-3.5 border border-slate-200 dark:border-white/[0.07]">
                            ...
                            <div className="text-xs text-slate-400">{u.order_count} sukses · {u.cancel_count} cancel</div>
                            <div className="text-xs text-orange-500 font-semibold mt-1">Cancel rate: {u.cancel_rate}%</div>
                          </div>
                          <div className="bg-white dark:bg-[#0d1020] rounded-2xl p-3.5 border border-slate-200 dark:border-white/[0.07]">
                            <div className="text-xs text-slate-400 mb-1">Net Spend (bersih)</div>
                            <div className="font-black text-orange-600 dark:text-orange-400">{fmtIDR(u.net_spend)}</div>
                            <div className="text-xs text-slate-400">setelah dikurangi refund</div>
                          </div>
                          <div className="bg-white dark:bg-[#0d1020] rounded-2xl p-3.5 border border-slate-200 dark:border-white/[0.07]">
